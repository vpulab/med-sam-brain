...
class TwoWayAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        q = queries + query_pe
        k = keys + key_pe
        attn_out = self.cross_attn_token_to_image(q=q, k=k, v=keys)#lora k
        queries = queries + attn_out
        ...
        q = queries + query_pe
        k = keys + key_pe
        attn_out = self.cross_attn_image_to_token(q=k, k=q, v=queries)#lora q
        keys = keys + attn_out
...
class AttentionForTwoWayAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = nn.Linear(embedding_dim, self.internal_dim)#lora 
